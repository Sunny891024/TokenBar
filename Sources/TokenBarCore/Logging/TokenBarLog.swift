...
public enum TokenBarLog {
    ...
    public static func logger(_ category: String) -> TokenBarLogger {
        let logger = Logger(label: "com.tokenbar.tokenbar.\(category)")
        return TokenBarLogger { level, message, metadata in
...
public struct TokenBarLogger: Sendable {
    private let logFn: @Sendable (TokenBarLog.Level, String, [String: String]?) -> Void

    fileprivate init(_ logFn: @escaping @Sendable (TokenBarLog.Level, String, [String: String]?) -> Void) {
        self.logFn = logFn
    }
