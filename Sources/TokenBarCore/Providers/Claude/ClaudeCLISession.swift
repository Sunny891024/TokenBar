...
actor ClaudeCLISession {
    ...
    private static let log = TokenBarLog.logger(LogCategories.claudeCLI)
    #if DEBUG
    ...
    private func ensureStarted(binary: String) throws {
        ...
        let disableWatchdog = ProcessInfo.processInfo.environment["TOKENBAR_DISABLE_CLAUDE_WATCHDOG"] == "1"
        if !disableWatchdog,
