...
public enum ClaudeOAuthCredentialsStore {
    ...
    static let log = TokenBarLog.logger(LogCategories.claudeUsage)
    ...
    /// - TokenBar-owned credentials refresh directly via token endpoint.
    ...
    /// Save refreshed credentials to TokenBar's keychain cache
    ...
    /// Check if TokenBar has cached credentials (in memory or keychain cache)
...
extension ClaudeOAuthCredentialsStore {
    ...
    /// TokenBar's caches. This is used to avoid triggering a second OS keychain dialog during the OAuth retry.
    @discardableResult
    ...
    /// Updates TokenBar's keychain cache with the new credentials.
