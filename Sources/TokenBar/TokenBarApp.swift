...
struct TokenBarApp: App {
    ...
    init() {
        ...
        TokenBarLog.bootstrapIfNeeded(.init(
            destination: .oslog(subsystem: "com.tokenbar.tokenbar"),
            level: level,
            json: false))
...
private func makeUpdaterController() -> UpdaterProviding {
    ...
    if InstallOrigin.isHomebrewCask(appBundleURL: bundleURL) {
        return DisabledUpdaterController(
            unavailableReason: "Updates managed by Homebrew. Run: brew upgrade --cask steipete/tap/tokenbar")
    }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let confettiLogger = TokenBarLog.logger(LogCategories.confetti)
