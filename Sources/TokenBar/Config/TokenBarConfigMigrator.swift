...
struct TokenBarConfigMigrator {
    ...
    static func loadOrMigrate(
        configStore: TokenBarConfigStore,
        userDefaults: UserDefaults,
        stores: LegacyStores) -> TokenBarConfig
    {
        let log = TokenBarLog.logger(LogCategories.configMigration)
        let existing = try? configStore.load()
        var config = (existing ?? TokenBarConfig.makeDefault()).normalized()
    ...
    private static func applyProviderOrder(_ raw: [String], config: TokenBarConfig) -> TokenBarConfig {
    ...
    private static func applyProviderToggles(
        ...
        config: TokenBarConfig) -> TokenBarConfig
    {
        var updated = config
