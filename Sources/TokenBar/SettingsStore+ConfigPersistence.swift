...
extension SettingsStore {
    private func updateConfig(reason: String, mutate: (inout TokenBarConfig) -> Void) {
    ...
    func updateProviderTokenAccounts(_ accounts: [UsageProvider: ProviderTokenAccountData]) {
        let summary = accounts
            ...
            .joined(separator: ",")
        TokenBarLog.logger(LogCategories.tokenAccounts).info(
    ...
    func reloadConfig(reason: String) {
        ...
        do {
            ...
            TokenBarLog.logger(LogCategories.configStore).error("Failed to reload config: \(error)")
        }
    }

    func applyExternalConfig(_ config: TokenBarConfig, reason: String) {
        guard !self.configLoading else { return }
    ...
    private func bumpConfigRevision(_ context: ConfigChangeContext) {
        ...
        TokenBarLog.logger(LogCategories.settings)
    ...
    func schedulePersistConfig() {
        ...
        if Self.isRunningTests {
            do {
                ...
                TokenBarLog.logger(LogCategories.configStore).error("Failed to persist config: \(error)")
            }
            return
        }
