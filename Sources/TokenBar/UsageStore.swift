...
final class UsageStore {
    ...
    @ObservationIgnored private let sessionQuotaLogger = TokenBarLog.logger(LogCategories.sessionQuota)
    @ObservationIgnored let openAIWebLogger = TokenBarLog.logger(LogCategories.openAIWeb)
    @ObservationIgnored private let tokenCostLogger = TokenBarLog.logger(LogCategories.tokenCost)
    @ObservationIgnored let augmentLogger = TokenBarLog.logger(LogCategories.augment)
    @ObservationIgnored let providerLogger = TokenBarLog.logger(LogCategories.providers)
...
extension UsageStore {
    func debugDumpClaude() async {
        ...
        let url = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("tokenbar-claude-probe.txt")
    ...
    func dumpLog(toFileFor provider: UsageProvider) async -> URL? {
        ...
        let filename = "tokenbar-\(provider.rawValue)-probe.txt"
