...
extension TokenBarCLI {
    static func runServe(_ values: ParsedValues) async {
        ...
        do {
            try await server.run {
                Self.writeStderr("TokenBar server listening on http://127.0.0.1:\(port)\n")
            }
        } catch {
    ...
    private static func serveUsage(
        ...
        config: TokenBarConfig) async -> CLILocalHTTPResponse
    {
    ...
    private static func serveCost(provider rawProvider: String?, config: TokenBarConfig) async -> CLILocalHTTPResponse {
        let selection: ProviderSelection
        do {
            selection = try Self.serveProviderSelection(rawProvider: rawProvider, config: config)
        } catch {
    ...
    private static func serveProviderSelection(
        rawProvider: String?,
        config: TokenBarConfig) throws -> ProviderSelection
    {
