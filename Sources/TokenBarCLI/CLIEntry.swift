...
enum TokenBarCLI {
    ...
    private static func bootstrapLogging(path: [String], values: ParsedValues) {
        TokenBarLog.bootstrapIfNeeded(self.loggingConfiguration(path: path, values: values))
    }

    static func loggingConfiguration(path: [String], values: ParsedValues) -> TokenBarLog.Configuration {
        ...
        let destination: TokenBarLog.Destination = path == ["diagnose"] ? .discard : .stderr
        return .init(destination: destination, level: level, json: isJSON)
    }

    static func resolvedLogLevel(verbose: Bool, rawLevel: String?) -> TokenBarLog.Level {
        TokenBarLog.parseLevel(rawLevel) ?? (verbose ? .debug : .error)
    }
