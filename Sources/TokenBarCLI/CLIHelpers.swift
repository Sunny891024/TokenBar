...
extension TokenBarCLI {
    static func decodeProvider(from values: ParsedValues, config: TokenBarConfig) -> ProviderSelection {
    ...
    static func resetTimeDisplayStyleFromDefaults() -> ResetTimeDisplayStyle {
        let domains = [
            "com.tokenbar.tokenbar",
            "com.tokenbar.tokenbar.debug",
        ]
    ...
    static func loadConfig(output: CLIOutputPreferences) -> TokenBarConfig {
        let store = TokenBarConfigStore()
        do {
            if let existing = try store.load() {
                return existing
            }
            return TokenBarConfig.makeDefault()
        } catch {
