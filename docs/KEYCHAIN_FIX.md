...
- Keychain is still used for runtime cache entries (for example `com.tokenbar.tokenbar.cache`) and Claude OAuth
...
| `ClaudeOAuthCredentials.swift` migrated TokenBar-owned Claude OAuth keychain items | Claude OAuth primary source is Claude CLI keychain service (`Claude Code-credentials`), with TokenBar cache in `com.tokenbar.tokenbar.cache` (`oauth.claude`). |
...
`Sources/TokenBar/KeychainMigration.swift` migrates legacy `com.tokenbar.TokenBar` items (for example
...
2. In-memory cache.
3. TokenBar keychain cache (`com.tokenbar.tokenbar.cache`, account `oauth.claude`).
...
When TokenBar does not have usable OAuth credentials in its own cache (`com.tokenbar.tokenbar.cache` / `oauth.claude`),
...
- Browser-imported Claude session cookies are cached in keychain service `com.tokenbar.tokenbar.cache`.
...
- Keychain cache store (`com.tokenbar.tokenbar.cache`) also writes with `ThisDeviceOnly`.
...
```bash
defaults read com.tokenbar.tokenbar KeychainMigrationV1Completed
```

### Check Claude OAuth keychain cooldown
```bash
defaults read com.tokenbar.tokenbar claudeOAuthKeychainDeniedUntil
```

### Inspect keychain-related logs
```bash
log show --predicate 'subsystem == "com.tokenbar.tokenbar" && (category == "keychain-migration" || category == "keychain-preflight" || category == "keychain-prompt" || category == "keychain-cache" || category == "claude-usage" || category == "cookie-cache")' --last 10m
```

### Reset migration for local testing
```bash
defaults delete com.tokenbar.tokenbar KeychainMigrationV1Completed
