...
**Files:**
- Create: `Sources/TokenBarCore/Providers/Kilo/KiloOrganization.swift`
- Create: `Tests/TokenBarTests/KiloOrganizationTests.swift`
...
```bash
git add Sources/TokenBarCore/Providers/Kilo/KiloOrganization.swift \
        Tests/TokenBarTests/KiloOrganizationTests.swift
...
**Files:**
- Create: `Sources/TokenBarCore/Providers/Kilo/KiloUsageScope.swift`
- Modify: `Tests/TokenBarTests/KiloOrganizationTests.swift`

- [ ] **Step 2.1: Append failing tests**

Append to `Tests/TokenBarTests/KiloOrganizationTests.swift`:
...
```bash
git add Sources/TokenBarCore/Providers/Kilo/KiloUsageScope.swift \
        Tests/TokenBarTests/KiloOrganizationTests.swift
...
**Files:**
- Modify: `Sources/TokenBarCore/Providers/Kilo/KiloUsageFetcher.swift`
- Modify: `Tests/TokenBarTests/KiloUsageFetcherTests.swift`

- [ ] **Step 3.1: Append failing test for header injection**

Append the following inside `struct KiloUsageFetcherTests` in `Tests/TokenBarTests/KiloUsageFetcherTests.swift`:
...
In `Sources/TokenBarCore/Providers/Kilo/KiloUsageFetcher.swift`:
...
```bash
git add Sources/TokenBarCore/Providers/Kilo/KiloUsageFetcher.swift \
        Tests/TokenBarTests/KiloUsageFetcherTests.swift
...
Append to `Sources/TokenBarCore/Providers/Kilo/KiloUsageFetcher.swift` (inside the `KiloUsageFetcher` struct):
...
**Files:**
- Modify: `Sources/TokenBarCore/Config/TokenBarConfig.swift`
- Tests: covered indirectly via Task 6 SettingsStore tests.

- [ ] **Step 5.1: Add fields to `ProviderConfig`**

In `Sources/TokenBarCore/Config/TokenBarConfig.swift`, inside `public struct ProviderConfig`:
...
```bash
git add Sources/TokenBarCore/Config/TokenBarConfig.swift
...
**Files:**
- Modify: `Sources/TokenBar/Providers/Kilo/KiloSettingsStore.swift`
- Create: `Tests/TokenBarTests/KiloSettingsStoreTests.swift`
...
Append to `Sources/TokenBar/Providers/Kilo/KiloSettingsStore.swift`:
...
```bash
git add Sources/TokenBar/Providers/Kilo/KiloSettingsStore.swift \
        Tests/TokenBarTests/KiloSettingsStoreTests.swift
...
**Files:**
- Modify: `Sources/TokenBarCore/Providers/Kilo/KiloProviderDescriptor.swift`
- Modify: `Sources/TokenBar/UsageStore+Refresh.swift`
- Create: `Sources/TokenBar/Providers/Kilo/UsageStore+KiloOrgRefresh.swift`
...
In `Sources/TokenBar/UsageStore.swift`, find the closest place where Codex-related published properties live (e.g. near `codexAccountSnapshots`) and add:
...
In `Sources/TokenBar/UsageStore+Refresh.swift`, find the existing `tokenAccounts` fan-out block in `refreshProvider`:
...
```bash
git add Sources/TokenBar/UsageStore.swift \
        Sources/TokenBar/UsageStore+Refresh.swift \
        Sources/TokenBar/Providers/Kilo/UsageStore+KiloOrgRefresh.swift
...
**Files:**
- Modify: `Sources/TokenBar/StatusItemController+MenuCardModel.swift` (or the file that produces Kilo menu rows)
- Modify: `Sources/TokenBar/MenuDescriptor.swift` if needed
...
```bash
grep -n "case \\.kilo" Sources/TokenBar/StatusItemController*.swift Sources/TokenBar/Menu*.swift 2>&1 | head -15
```
...
```bash
git add Sources/TokenBar/StatusItemController+MenuCardModel.swift \
...
**Files:**
- Modify: `Sources/TokenBar/Providers/Kilo/KiloProviderImplementation.swift`
- Possibly modify: `Sources/TokenBar/PreferencesProviderDetailView.swift` and `Sources/TokenBarCore/Providers/ProviderDescriptor.swift` if a new descriptor variant is needed.
...
grep -n "enum Kind\|case info\|case toggleList\|case checkboxList" \
    ...
    Sources/TokenBar/PreferencesProviderDetailView.swift 2>&1 | head -20
```

If a toggle-list kind exists, reuse it. Otherwise, add a new `ProviderSettingsOrganizationsDescriptor` in `Sources/TokenBarCore/Providers/ProviderDescriptor.swift`:
...
In `Sources/TokenBar/Providers/Kilo/KiloProviderImplementation.swift`:
...
In `Sources/TokenBar/PreferencesProviderDetailView.swift`, follow the existing pattern used by `settingsTokenAccounts`. Add a stored property `settingsOrganizations: ProviderSettingsOrganizationsDescriptor?`, source it from the provider implementation, and render it as a SwiftUI section under the API key:
...
```bash
git add Sources/TokenBarCore/Providers/ProviderDescriptor.swift \
        Sources/TokenBar/Providers/Kilo/KiloProviderImplementation.swift \
...
```bash
git remote get-url fork 2>/dev/null || git remote add fork https://github.com/noefabris/TokenBar.git
