...
- Updater: `SPUStandardUpdaterController` owned by `AppDelegate` (see `Sources/TokenBar/TokenbarApp.swift:1`).
...
## Release flow
1) Build & notarize as usual (`./Scripts/sign-and-notarize.sh`), producing notarized `TokenBar-macos-universal-<ver>.zip`.
