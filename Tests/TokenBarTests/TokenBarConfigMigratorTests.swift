...
struct TokenBarConfigMigratorTests {
    ...
    func `legacy secret migration completion flag skips repeated scans`() throws {
        let suite = "TokenBarConfigMigratorTests-skip-\(UUID().uuidString)"
        ...
        _ = TokenBarConfigMigrator.loadOrMigrate(configStore: configStore, userDefaults: defaults, stores: stores)
    ...
    func `legacy migration completion waits for successful cleanup`() throws {
        let suite = "TokenBarConfigMigratorTests-cleanup-failure-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
        defer { defaults.removePersistentDomain(forName: suite) }
    ...
    func `legacy stores are kept when migrated config save fails`() throws {
        let suite = "TokenBarConfigMigratorTests-save-failure-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
        defer { defaults.removePersistentDomain(forName: suite) }
        ...
        let accountStore = CountingTokenAccountStore()
        let stores = Self.legacyStores(secrets: secrets, accountStore: accountStore)
        let configStore = TokenBarConfigStore(
    ...
    private static func legacyStores(
        ...
        accountStore: CountingTokenAccountStore) -> TokenBarConfigMigrator.LegacyStores
    {
