...
struct LocalizationBundleTests {
    ...
    func `packaged app resolves localization bundle from resources`() throws {
        ...
        let bundle = tokenBarLocalizationResourceBundle(mainBundle: fixture.appBundle)
    ...
    func `empty localized values fall back to English`() throws {
        ...
        let resourceBundle = tokenBarLocalizationResourceBundle(mainBundle: fixture.appBundle)
        let zhPath = try #require(resourceBundle.path(forResource: "zh-Hans", ofType: "lproj"))
        let zhBundle = try #require(Bundle(path: zhPath))

        #expect(tokenBarLocalizedString("Settings", bundle: zhBundle, resourceBundle: resourceBundle) == "Settings")
        #expect(tokenBarLocalizedString("Missing", bundle: zhBundle, resourceBundle: resourceBundle) == "Missing")
    }
        ...
        includeEmptyChineseLocalization: Bool = false) throws -> (root: URL, appBundle: Bundle)
    {
        ...
        <dict>
            ...
            <key>CFBundleIdentifier</key><string>com.tokenbar.tokenbar.tests</string>
